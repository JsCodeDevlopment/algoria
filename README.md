...
- **Node.js** 18+
- **pnpm** 9+ (recomendado; ver `packageManager` em `package.json`)
...
Cria um `.env.local` (ou exporta no teu ambiente) conforme necessário:

```env
# Ambiente: em produção REAL combina com NODE_ENV=production para sitemap/robots públicos
NEXT_PUBLIC_ENVIRONMENT="development"

# Domínio canónico (sitemap e robots); fallback interno: algoria.app
NEXT_PUBLIC_APP_DOMAIN="https://algoria.app"

# PostHog — opcional; sem chave o provider não envia eventos
NEXT_PUBLIC_POSTHOG_KEY=""
NEXT_PUBLIC_POSTHOG_HOST="https://eu.i.posthog.com"
```

> **Indexação:** com `NEXT_PUBLIC_ENVIRONMENT=production` e `NODE_ENV=production`, `robots.ts` e `sitemap.ts` permitem crawling público. Em `development` ou `staging`, o `robots.txt` tende a bloquear (`disallow: /`) — útil para previews.
...
| `pnpm validate:content` | Valida metadados Zod, soluções, conceitos e guias editoriais |
| `pnpm sync:player-annotations` | Ferramenta auxiliar de anotações (ver script) |
| `pnpm bootstrap:phase1` | Bootstrap de conteúdo fase 1 (ver script) |

Type-check rápido (se precisares):
...
- [LEETCODE_LEARNING_APP_PLAN.md](./LEETCODE_LEARNING_APP_PLAN.md) — plano editorial e roadmap alargado
- [AGENTS.md](./AGENTS.md) — orientações para contribuição com agentes / particularidades do Next.js do projeto
