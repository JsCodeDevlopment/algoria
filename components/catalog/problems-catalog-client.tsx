      ...
      <div className="mx-auto max-w-7xl px-6 py-24 flex-1">
        <header className="mb-16 border-l-4 border-primary pl-8">
          ...
          <p className="max-w-2xl text-lg leading-relaxed tracking-tight text-muted-foreground">
            Filtra por dificuldade e categoria, pesquisa por título e segue a
            ordem recomendada. O progresso fica no teu browser; sincroniza na
            nuvem com conta Pro.
          </p>
        ...
        <p className="mb-6 text-xs text-muted-foreground font-mono uppercase tracking-tight">
          {filteredSorted.length} problema
                  ...
                  <div className="flex items-center gap-2 mb-4 flex-wrap">
                    ...
                    {p.categories.slice(0, 2).map((c) => (
                      <Badge
                        key={c}
                        variant="secondary"
                        className="font-mono text-[9px] px-1.5 py-0 rounded-none bg-primary/10 text-primary"
                      >
                        {categoryLabelPt(c as Category)
                          .replace(/\s+/g, "_")
                          .toUpperCase()}
                      </Badge>
                    ))}
